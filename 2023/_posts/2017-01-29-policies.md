...
Recitations are held once a week. These sessions are led by pairs of graduate and/or undergraduate TAs, and are designed to help guide your thinking on problem sets, and further explore topics introduced in class. Feel free to attend whichever is most convenient, within room capacity limits.

Tutorials are also held once a week, hosted by the head TAs. Computational skills are increasingly important in the modern biological era; thus many homework problems will ask you to write code to perform calculations, simulations, and analyses. Since no prior programming experience is required for the course, these sessions are intended to help bring you up to speed with the week's relevant computational topics. See the [programming page]({{ site.baseurl }}/2023/programming) for details.
...
To ensure proper grading, all homework must be submitted via Gradescope and adhere to the following conventions:
- For text solutions, submit a PDF with the filename `lastname_firstname_hw#.pdf`. This can be scanned handwritten work, but LaTeX is preferred.
- For code solutions, submit a Jupyter Notebook with the filename `lastname_firstname_hw#.ipynb`, along with any images used in the Notebook. If you write your entire homework in a Notebook, you do not need to submit a PDF.

<!-- code submission gradescope template? make page selection req'd -->
...
- Each regrade request applies to one assignment. In other words, if you ask for a regrade, the entire homework will be subjected to the same high level of scrutiny you have requested for the problem you feel was not graded properly. Keep in mind that this could result in a net reduction in your grade on that particular assignment, if your previous marks on other problems are adjusted.
- Regrades may be used to resubmit improperly submitted assignments, but the late policy still applies.
- Regrades are final.

To request a regrade, email `bi1.caltech@gmail.com` with the subject line `[Homework #] Regrade Request for Firstname Lastname`. You may provide a description of any mistakes you feel were made in the body of the email.
